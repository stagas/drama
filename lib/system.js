...
ActorSystem.prototype.deliver = function (message) {
  ...
  if (!message.to) return console.error('No sender', message)
...
ActorSystem.prototype.fork = function (address, bus) {
  var sys = this
  bus = bus || cp.fork(__dirname + '/worker.js')
  ...
  remote.send = function (message) {
    ...
    this.bus.send(utils.safe(message))
  }
  ...
  remote.create = function (address, beh) {
    ...
    remote.ctrl.create(address, toSource(beh))
    return sys.ref(remote.address + '@' + address)
  }

  this.router.add(remote)

  remote.bus.on('message', function (message) {
    //console.log('got message from bus', message)
    sys.deliver(message)
  })
